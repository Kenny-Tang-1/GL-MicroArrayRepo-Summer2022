...
compare_csv <- compare_csv_from_runsheet("Runsheet(GLDS-41).csv")
...
result_table <- topTable(contrast.fit, number = 9000000) # TODO: Replace this with a better way of obtaining the table.
```
...
# Join tables
bound_columns <- bind_cols(norm_data_filtered$E, norm_data_filtered$genes$ProbeName) %>% rename("ProbeName" = "...19")
compiled_table <- result_table %>% left_join(unique_probe_ids, by = c("ProbeName" = "agilent_020186")) %>% left_join(bound_columns) 
write.csv(compiled_table, file = file.path(dir, "/output_data/GLDS-41_output_data/Probe_level_annotated_DGE_with_Normalized_Intensities.csv"), row.names = FALSE) 
```
