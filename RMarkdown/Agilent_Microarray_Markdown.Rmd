---
title: "Agilent Microarray Processing Pipeline RMarkdown"
...
For processing Agilent microarray data, we will need to install a few packages that will used for the importing and analyzing the data. These will include Bioconductor as well as Linear Models for Microarray (Limma). These are popular packages for processing microarray data.
...
Now that we have our tools to look at the data, we can start by importing the raw data from the ".txt" files into R.
```{r}
GLDS_41_rs <- read.csv("Runsheet(GLDS-41).csv", check.names = FALSE, fileEncoding = 'UTF-8-BOM') ## Outputs a dataframe
Factor_Value <- x[,"Factor Value[gravity]"]
```
```{r Raw_Data_Input} 
