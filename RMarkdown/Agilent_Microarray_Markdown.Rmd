...
compare_csv <- compare_csv_from_runsheet(file.path(dir, "Runsheet(GLDS-41).csv"))
...
## Input: ".txt" files, Output: EListRaw (Raw intensities)
raw_data <- limma::read.maimages(files, source="agilent", path = file.path(datadir, "GLDS-41/GLDS-41_Raw_Data"), sep="\t", green.only = TRUE, other.columns="gIsWellAboveBG")
```
...
# Some not a number (NaNs) errors appeared (possibly due to log transformations), but overall doesn't obscure the plot so I suppressed the warnings.
```
## TODO: Track the NaNs (Not a Number errors)
...
for (sample_name in colnames(raw_data$E)) {
    if (length(raw_data$printer) != 0)  {
        ...
        imageplot(raw_data$E[,sample_name], layout=list(ngrid.r = 1, ngrid.c = 1, nspot.r = 532, nspot.c = 85), zlim = c((1.25*min(raw_data$E[,sample_name])),(0.75*max(raw_data$E[,sample_name]))), legend = TRUE, main = sample_name)
        }
}
```
...
# There were NaN errors popping up due to the log transformations going on, but they didn't obscure the plots generated, so I went ahead and suppressed the warnings.
```

- raw_data = EListRaw object containing the raw expression values and information about the arrays
...
```{r Normalized_density_plot, fig.height = 8, fig.width = 8, fig.align = "center"}
...
control_probes <- norm_data$genes$ControlType==1L # Removes control probes
IsExpr <- rowSums(norm_data$other$gIsWellAboveBG > 0) >= 4 # Removes probes that don't appear to be expressed
unmapped_probes <- raw_probe_level_data_df$GeneName == raw_probe_level_data_df$ProbeName # Removes probes where the ProbeName is also the GeneName
...
# TODO: 1. See if the values are pairwise values are Log2FC (rename it if so), 2. If not, figure out how to get Log2FC
...
compiled_table <- result_table %>% left_join(unique_probe_ids, by = c("ProbeName" = "agilent_020186")) %>% left_join(bound_columns) 
write.csv(compiled_table, file = file.path(dir, "/output_data/GLDS-41_output_data/Probe_level_annotated_DGE_with_Normalized_Intensities_GLDS_41.csv"), row.names = FALSE) 
```
...
- compiled_table = Table that contains the merged DGE, normalized intensites, and gene annotations.
  - by = c("ProbeName" = "agilent_020186") - Telling the function left_join how to join the two tables. In this case we're telling the function to use the ProbeName column from result_table and the "agilent_020186" column (probe names) from unique_probe_ids. 
