...
I moved on to removing a lot of the technical variation in the raw intensities
...
## Generating Probe and Gene (expression) Level Data
```{r Generating_Probe_Level_and_Expression_Level_Data}
...
first_sample_raw = paste0(colnames(raw_data$E)[1], "_mean")
sample_one_raw = colnames(raw_data$E)[1]
summarized_columns_raw <- raw_probe_level_data_df %>% group_by(GeneName) %>% summarize(!!first_sample_raw := mean(get(sample_one_raw))) # Using this data frame to merge with
...
write.csv(norm_probe_level_data_df, file = file.path(dir, "/output_data/GLDS-41_output_data/Normalized_probe_level_data.csv"), row.names = FALSE)
...
write.csv(summarized_columns_norm, file = file.path(dir, "/output_data/GLDS-41_output_data/Normalized_gene_level_data.csv"), row.names = FALSE)
## TODO: priority
##   Export Raw probe level data (x)
##   Generate Raw gene-summarization level data (include columns that are one-to-one with Gene info) (x)
##   Repeat for normalized expression (remove controls)
```

```{r Filtering_Out_Controls_From Normalized_Data}
control_probes <- norm_data$genes$ControlType==1L
no_gene_name <- is.na(norm_data$genes$GeneName)
norm_data_filtered <- norm_data[!control_probes & !no_gene_name, ]
```
