...
compare_csv <- compare_csv_from_runsheet(file.path(dir, "Runsheet(GLDS-174).csv"))
...
suppressWarnings(plotDensities(raw_data, log = TRUE, legend = "topright" , main = "Density of raw expression values for multiple arrays"))
...
```{r Normalized_density_plot, fig.height = 15, fig.width = 15, fig.align = "center"}
...
searchDatasets(mart = ensembl, pattern = "Human") # Pattern = organism of interest
ensembl <- useDataset(dataset = "hsapiens_gene_ensembl", mart = ensembl) # Selecting the dataset
...
results <- getBM(
    attributes = c(
        "agilent_wholegenome_4x44k_v2", # Name of the array from the ADF file (Write a markdown file about how I found the array name)
        "ensembl_gene_id",
        "uniprot_gn_symbol",
        "go_id"
...
unique_probe_ids <- results %>% group_by(agilent_wholegenome_4x44k_v2) %>%
                      summarise(
...
compiled_table <- result_table %>% left_join(unique_probe_ids, by = c("ProbeName" = "agilent_wholegenome_4x44k_v2")) %>% left_join(bound_columns) 
write.csv(compiled_table, file = file.path(dir, "/output_data/GLDS-174_output_data/Probe_level_annotated_DGE_with_Normalized_Intensities_GLDS_174.csv"), row.names = FALSE) 
```
...
- searchDatasets = Lists out the datasets available in the database that follow the specific pattern indicated
  - pattern = "Human" = Specifying which organism dataset we want to look for in the database.
- dataset = "hsapiens_gene_ensembl" - Choosing the dataset from the list of available datasets for that organism.
...
- compiled_table = Table that contains the merged DGE, normalized intensites, and gene annotations.
  - by = c("ProbeName" = "agilent_wholegenome_4x44k_v2") - Telling the function left_join how to join the two tables. In this case we're telling the function to use the ProbeName column from result_table and the "agilent_020186" column (probe names) from unique_probe_ids. 
